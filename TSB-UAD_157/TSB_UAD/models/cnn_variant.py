...
class CnnVariant:
    ...
    def fit(self, X_clean, X_dirty, ratio = 0.15):
        ...
        for i in range(0, len(X_test), 64):
            current_X_test_batch = X_test[i:i + 64]
            current_Y_test_batch = Y_test[i:i + 64]
            prediction.extend(model.predict(current_X_test_batch))
            model.train_on_batch(current_X_test_batch, current_Y_test_batch)
