...
class POLYVariant(DetectorA):
    ...
    def decision_function(self, X= False, measure = None):
        ...
        for i in range(M, N+1):
            ...
            current_scores_per_window = []
            for model in self.estimator.keys():
                internal_index = int(model.replace('model', ''))

                if internal_index <= index:
                    current_scores_per_window.append(measure.measure(
                        self.X_train_[internal_index: internal_index + (window if end != self.n_train_ else self.n_train_)], 
                        estimation[internal_index: internal_index + (window if end != self.n_train_ else self.n_train_)], 
                        internal_index
                    ))


            score[index: index+window] = np.mean(np.array(current_scores_per_window), axis=0).tolist()
