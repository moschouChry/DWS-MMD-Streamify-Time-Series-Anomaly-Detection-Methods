...
class HBOSVariant(DetectorB):
    """Histogram- based outlier detection variant (HBOS Variant) is an efficient unsupervised
    method. It assumes the feature independence and calculates the degree
    of outlyingness by building streaming histograms. See :cite:`goldstein2012histogram`
    for details.
    Parameters
    ----------
    ...
    def __init__(self, n_features, n_bins=10, alpha=0.1, tol=0.5, contamination=0.1):
        super(HBOSVariant, self).__init__(contamination=contamination)
        self.n_bins = n_bins
        self.alpha = alpha
        self.tol = tol
        self.model_name = 'HBOS variant'
        self.distogram_per_dimension = {}
    ...
    def fit(self, X, y=None):
        ...
        for i in range(self.n_features):
            if i not in self.distogram_per_dimension:
                self.distogram_per_dimension[i] = distogram.Distogram(self.n_bins)

            for sample in X[:, i]:
                self.distogram_per_dimension[i] = distogram.update(self.distogram_per_dimension[i], sample)

            distogram_result = distogram.histogram(self.distogram_per_dimension[i], bin_count=self.n_bins)
            
            # convert to numpy array from list
            distogram_result = np.array(distogram_result[0]), np.array(distogram_result[1])

            n = distogram_result[0]
            bin_edges = distogram_result[1]
            # same as density = True in numpy.histogram
            db = np.array(np.diff(bin_edges), float)
            distogram_result = n/db/n.sum(), bin_edges

            self.hist_[:, i], self.bin_edges_[:, i] = distogram_result
