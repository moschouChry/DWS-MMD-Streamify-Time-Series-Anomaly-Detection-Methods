...
def sigmoid_derivative(x, center=0):
    sig = scaled_and_centered_sigmoid(x, center=center)
    return sig * (1 - sig)

def scaled_and_centered_sigmoid(x, a=0, b=100, center=0):
    return a + (b - a) * sigmoid(x - center)

class POLYVariant(DetectorA):
    ...
    def fit(self, X, y=None):
        ...
        for i in range(M, N+1):
            ...
            # assert (last_index != -1) if first_iteration != True else (last_index == -1)
            if index + neighbor < self.n_train_ and index - neighbor > 0: 
                # assert first_iteration == False
                ...
                if first_iteration:
                    # new_y = []
                    # for current_value in y:
                    #     new_y.append(scaled_and_centered_sigmoid(current_model(current_x), center=(x[-1] - x[0])//2))
            ...
            if not first_iteration:
                copied_model = copy.deepcopy(fit['model' + str(last_index)])
                previous_w = copied_model.c

                for current_x, y_i_actual in zip(x, y):
                    current_model = np.poly1d(previous_w)
                    y_i_estimate = scaled_and_centered_sigmoid(current_model(current_x))

                    delta_w = sigmoid_derivative(y_i_actual) * current_model(current_x) * (y_i_actual - y_i_estimate)

                    new_w = previous_w + learning_rate * delta_w
                    previous_w = new_w.copy()

                
                fit['model' + str(index)] = np.poly1d(new_w)
        ...
        for i in range(M, N):
            ...
            estimation = fit['model' + str(i * window)](myline)
            Y[window *i: window * (i+1)] = [scaled_and_centered_sigmoid(current_estimation) for current_estimation in estimation]
